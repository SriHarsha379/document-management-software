...
- principalCompany: The principal/client company name on the LR — the company whose goods are being transported. Look first for an explicit label such as "Principal Company:", "Principal:", or "Company:". If no label is present, extract the most prominent company name from the document header (the consignor/sender company at the top of the document). This is the client company, NOT the transport/carrier company. Return the full name exactly as printed (e.g. "MY HOME INDUSTRIES PRIVATE LIMITED"). Return null only if no company name can be identified.
- branchName: The branch of the principal/issuing company on the LR. Look first for an explicit "Branch:" label anywhere in the document header area and use that value normalized to UPPERCASE (e.g. "DRONAGIRI BRANCH" → "DRONAGIRI BRANCH"). If no explicit "Branch:" label exists, extract the most specific locality or area name from the company/consignor header address block, normalized to UPPERCASE (e.g. the locality token before the city, such as "DRONAGIRI" from an address like "Dronagiri, Navi Mumbai"). Do not use a generic city or district name as the branch value when a more specific locality is present. Return null if no branch or locality can be identified.
