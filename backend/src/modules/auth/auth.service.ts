...
if (!rawSecret) {
  ...
  console.warn('[auth] WARNING: JWT_SECRET is not set — using an insecure default. Set JWT_SECRET before deploying.');
}

// Guaranteed non-undefined from here down
const JWT_SECRET: string = rawSecret ?? 'change-me-in-development-only';

// Accept any string that jwt treats as a duration (e.g. '8h', '1d', '3600')
...
// access, and source access from the database.  Embed the result in the JWT so
// subsequent requests do not require extra DB round-trips.
...
export async function loadUserContext(userId: string): Promise<UserContext | null> {
  ...
  // De-duplicate permissions from all assigned roles
